        ...
        if (model.isQuad()) { /* If the model is quadrangulated */
            if (nn == nt && nn == nv && nt == nv) {
                for (int i = 0; i < nn; i += 4) {
                    cout << "n: " << normals->at(i) << " " << normals->at(i + 1)
                         << " " << normals->at(i + 2) << " "
                         << normals->at(i + 3) << endl;
                    cout << "t: " << tex_coords->at(i) << " "
                         << tex_coords->at(i + 1) << " "
                         << tex_coords->at(i + 2) << " "
                         << tex_coords->at(i + 3) << endl;
                    cout << "v: " << vertices->at(i) << " "
                         << vertices->at(i + 1) << " " << vertices->at(i + 2)
                         << " " << vertices->at(i + 3) << endl;
                }
            } else {
                for (int i = 0; i < nn; i += 4) {
                    cout << "n: " << normals->at(i) << " " << normals->at(i + 1)
...
void PrintModelMaterialInfo(Model &model) {
    cout << "Materials: " << model.Materials()->size() << endl;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        cout << "Using Model Loading Library Version " << version->MAJOR << "."
             << version->MINOR << endl
             << endl;
